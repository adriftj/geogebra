...
public class GpadParserTest extends BaseUnitTest {
	...
	// ========== Tests for optional trailing semicolon in style sheet ==========

	@Test
	public void testStyleSheetWithTrailingSemicolonNoWarning() {
		// Test that style sheet with trailing semicolon doesn't generate warnings
		// This verifies the fix at line 2589 in Parser.jj which allows optional trailing semicolon
		String gpad = "@style1 = { pointSize: 5; fixed; }\nA @style1 = (1, 2)";
		GpadParser parser = new GpadParser(getKernel());
		...
		try {
			...
			GeoElement geo = geos.get(0);
			assertEquals("A", geo.getLabelSimple());
			// Verify that style sheet was registered
			assertTrue(parser.getGlobalStyleSheets().containsKey("style1"));
			// Verify no warnings related to trailing semicolon were generated
			// The fix at line 2589 allows optional trailing semicolon, so it should not
			// trigger error recovery that would generate warnings about skipped tokens
			List<String> warnings = parser.getGpadWarnings();
			assertTrue("Should not generate warnings", 
	...
	@Test
	public void testStyleSheetWithoutTrailingSemicolonNoWarning() {
		// Test that style sheet without trailing semicolon also works (baseline)
		String gpad = "@style1 = { pointSize: 5; fixed }\nA @style1 = (1, 2)";
		GpadParser parser = new GpadParser(getKernel());
	...
	@Test
	public void testInlineStyleSheetWithTrailingSemicolonNoWarning() {
		// Test inline style sheet with trailing semicolon
		String gpad = "A { pointSize: 6; fixed; } = (1, 2)";
		GpadParser parser = new GpadParser(getKernel());
	...
	@Test
	public void testStyleSheetMultiplePropertiesWithTrailingSemicolonNoWarning() {
		// Test style sheet with multiple properties and trailing semicolon
		// Verifies that the fix at line 2589 in Parser.jj works for complex style sheets
		String gpad = "@style1 = { pointSize: 5; fixed; lineStyle: thickness=3; }\nA @style1 = (1, 2)";
		GpadParser parser = new GpadParser(getKernel());
	...
	@Test
	public void testEmptyStyleSheetNoWarning() {
		// Test completely empty style sheet {} should parse without warnings or errors
		String gpad = "@style1 = { }\nA @style1 = (1, 2)";
		GpadParser parser = new GpadParser(getKernel());
		...
		try {
			...
			// Verify no warnings were generated
			List<String> warnings = parser.getGpadWarnings();
			assertTrue("Should not generate warnings for empty style sheet", 
					warnings == null || warnings.isEmpty());
		} catch (GpadParseException e) {
			throw new AssertionError("Parse failed: " + e.getMessage(), e);
		}
	...
	@Test(expected = GpadParseException.class)
	public void testStyleSheetWithOnlySemicolonShouldFail() throws GpadParseException {
		// Test style sheet with only semicolon { ; } should be a syntax error
		String gpad = "@style1 = { ; }\nA @style1 = (1, 2)";
		GpadParser parser = new GpadParser(getKernel());
		parser.parse(gpad);
		// Should throw GpadParseException
	}
}
